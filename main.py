...
sys.path.append("..")
from common.download_utils import download_resources
...
# The next additional functions help to create the mapping between tokens and ids for a sentence.
...
def generate_batches(batch_size, tokens, tags,
...
def declare_placeholders(self):
    ...
    # Placeholder for a dropout keep probability. If don't feed
...
def compute_predictions(self):
    ...
    # Set axis=-1 otherwise argmax will be calculated in a wrong way
...
def perform_optimization(self):
    ...
    # Gradient clipping for self.grads_and_vars
    # Apply this operation only for gradients because self.grads_and_vars contains also variables.
...
def eval_conll(model, session, tokens, tags, short_report=True):
    ...
    y_true, y_pred = [], []
    for x_batch, y_batch, lengths in generate_batches(1, tokens, tags):
        ...
        # Extend every prediction and ground truth sequence with 'O' tag
...
for epoch in range(n_epochs):
    ...
    # Train the model
    for x_batch, y_batch, lengths in generate_batches(batch_size, train_tokens, train_tags):
